...
- **Starting a service with High or RealTime priority on Windows** — On Windows, starting a game service or a virtual server whose process priority was set to *High* or *RealTime* failed with a "required privilege is not held by the client" error, while *AboveNormal* and lower worked.
